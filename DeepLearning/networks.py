...
def deeper_direct_CNN(x,y):
    ...
    cnn_layer1 = tf.keras.layers.Convolution2D(64, (3,3), padding='same')(input_layer)
    layer1_activation = tf.keras.layers.LeakyReLU()(cnn_layer1)
    cnn_layer2 = tf.keras.layers.Convolution2D(32, (3,3), padding='same')(layer1_activation)
    layer2_activation = tf.keras.layers.LeakyReLU()(cnn_layer2)
    output_layer = tf.keras.layers.Convolution2D(1, (3,3), padding='same', activation="sigmoid")(layer2_activation)
    cpd_model=tf.keras.models.Model(input_layer,output_layer)
